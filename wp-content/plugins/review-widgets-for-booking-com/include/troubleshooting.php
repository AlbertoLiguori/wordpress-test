...
$reviews = null;
if ($trustindex_pm_booking->is_table_exists('reviews')) {
$reviews = [];
...
$addCss = false;
if (method_exists($trustindex_pm_booking, 'getCssFile')) {
$addCss = true;
}
?>
...
<?php if ($addCss): ?>CSS path: <?php echo esc_html($trustindex_pm_booking->getCssFile()) ."\n"; ?><?php endif; ?>
PHP Info: <?php echo "\n\t"; ?>
...
Options: <?php foreach($trustindex_pm_booking->get_option_names() as $optName) {
if ($optName == 'css-content') {
continue;
}
$option = get_option($trustindex_pm_booking->get_option_name( $optName ));
echo "\n\t". esc_html($optName) .': ';
...
if ($addCss) {
echo "\n\nCSS: ". esc_html(get_option($trustindex_pm_booking->get_option_name('css-content')));
}
echo "\n\n";
?>
Active Theme: <?php
