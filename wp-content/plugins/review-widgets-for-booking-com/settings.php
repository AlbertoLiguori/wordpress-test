...
case 'later':
$trustindex_pm_booking->setNotificationParam($type, 'timestamp', time() + (14 * 86400));
break;
case 'close':
if ($type !== 'rate-us') {
$trustindex_pm_booking->setNotificationParam($type, 'active', false);
}
break;
case 'open':
$trustindex_pm_booking->setNotificationParam($type, 'active', false);
if ($type === 'rate-us') {
header('Location: https://wordpress.org/support/plugin/'. $trustindex_pm_booking->get_plugin_slug() . '/reviews/?rate=5#new-post');
exit;
}
$tab = 'setup_no_reg';
...
break;
case 'hide':
$trustindex_pm_booking->setNotificationParam($type, 'hidden', true);
header('Location: admin.php?page=' . sanitize_text_field($_GET['page']) .'&tab=advanced');
break;
case 'unhide':
$trustindex_pm_booking->setNotificationParam($type, 'hidden', false);
header('Location: admin.php?page=' . sanitize_text_field($_GET['page']) .'&tab=advanced');
break;
}
exit;
...
$tabs[ TrustindexPlugin_booking::___('Advanced') ] = 'advanced';
$tabs[ TrustindexPlugin_booking::___('Feature request') ] = 'feature_request';
$selectedTab = isset($_GET['tab']) ? sanitize_text_field($_GET['tab']) : null;
$subTabs = null;
$newBadgeTabs = [];
if (get_option($trustindex_pm_booking->get_option_name('widget-setted-up'), 0) && !get_option($trustindex_pm_booking->get_option_name('reply-generated'), 0)) {
...
$proxyCheck = true;
if ($proxy->is_enabled()) {
$optName = $trustindex_pm_booking->get_option_name('proxy-check');
...
<a href="?page=<?php echo esc_attr($_GET['page']); ?>&tab=<?php echo esc_attr($_GET['tab']); ?>&test_proxy" class="btn-text btn-refresh"><?php echo TrustindexPlugin_booking::___('Test again') ;?></a>
</div>
<?php endif; ?>
...
<span class="ti-new-badge"><?php echo TrustindexPlugin_booking::___('new'); ?></span>
<?php endif; ?>
</a>
<?php endforeach; ?>
</div>
