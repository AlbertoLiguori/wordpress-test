...
echo $trustindex_pm_airbnb->error_box_for_admins(TrustindexPlugin_airbnb::___("Please fill out <strong>all the required fields</strong> in the <a href='%s'>widget settings</a> page", [ admin_url('admin.php?page='.$trustindex_pm_airbnb->get_plugin_slug().'/settings.php') ]));
}
echo $after_widget;
}
else {
...
<?php if ($tiWidgets): ?>
<h2><?php echo TrustindexPlugin_airbnb::___('Your saved widgets'); ?></h2>
...
TrustindexPlugin_airbnb::___('You have no widget saved!') . ' '
. "<a target='_blank' href='" . "https://admin.trustindex.io/" . "widget'>". TrustindexPlugin_airbnb::___("Let's go, create amazing widgets for free!")."</a>"
); ?>
<?php endif; ?>
...
<?php echo TrustindexPlugin_airbnb::___($fparams['name']); ?> <?php if ($fparams['required']): ?><strong class="text-danger">*</strong><?php endif; ?>
<?php if ($fparams['help-icon']): ?>
<?php echo TrustindexPlugin_airbnb::___($fparams['help-icon']); ?>
<?php endif; ?>
</label>
<input
type="text"
placeholder="<?php echo TrustindexPlugin_airbnb::___($fparams['placeholder']); ?>"
...
/>
<?php if ($fparams['help']): ?>
<small class="text-muted"><?php echo TrustindexPlugin_airbnb::___($fparams['help']); ?></small>
<?php endif; ?>
</div>
</div>
<?php endforeach; ?>
...
<?php else: ?>
<?php echo TrustindexPlugin_airbnb::get_alertbox('warning',
TrustindexPlugin_airbnb::___('You have not set up your Trustindex account yet!') . ' ' .
TrustindexPlugin_airbnb::___('You can only list 10 reviews without it.') . '<br>'
. TrustindexPlugin_airbnb::___("Go to <a href='%s'>plugin setup page</a> to complete the one-step setup guide and enjoy the full functionalization!", [ admin_url('admin.php?page='.$trustindex_pm_airbnb->get_plugin_slug().'/settings.php&tab=setup_trustindex') ])
); ?>
<?php endif; ?>
</div>
<?php
